...
class UpdatePersonalDataProcessedAction
{
    ...
    public function execute(
        PersonalDataProcessedDto $dto,
        PersonalDataProcessed $personalDataProcessed
    ): ?PersonalDataProcessed {
        return DB::transaction(function () use ($dto, $personalDataProcessed) {
            /** @var User $user */
            $user = Auth::user();

            $personalDataProcessed->name = $dto->name;
            $personalDataProcessed->user_id = $user->id;

            return $this->personalProcessedData->save($personalDataProcessed);
        });
    }
}
